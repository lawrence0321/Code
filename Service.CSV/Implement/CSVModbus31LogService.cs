...
{
    internal class CSVModbus31LogService : ICSVModbus31LogService
    {
        readonly string FolderPath = CSVServiceFactory.BasicFolderPath + @"Modbus31Log\";

        static readonly object Token = new object();

        public ActResult Log(Modbus31LogDTO Modbus31Log_)
        {
            lock (Token)
            {
                try
                {
                    ...
                    if (!File.Exists(fileName))
                    {
                        ...
                        foreach (var par in typeof(Modbus31LogDTO).GetProperties())
                        {
                    ...
                    foreach (var par in typeof(Modbus31LogDTO).GetProperties())
                    {
                        ...
                        if (display.Visible)
                        {
                            try
                            {
                                str += string.Format(",{0}", par.GetValue(Modbus31Log_).ToString());
                            }
                            catch
                            {
                                str += string.Format(",{0}", "-");
