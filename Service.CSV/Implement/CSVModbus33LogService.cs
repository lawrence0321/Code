...
{
    internal class CSVModbus33LogService : ICSVModbus33LogService
    {
        readonly string FolderPath = CSVServiceFactory.BasicFolderPath + @"Modbus33Log\";

        static readonly object Token = new object();

        public ActResult Log(Modbus33LogDTO Modbus33Log_)
        {
            lock (Token)
            {
                try
                {
                    ...
                    if (!File.Exists(fileName))
                    {
                        ...
                        foreach (var par in typeof(Modbus33LogDTO).GetProperties())
                        {
                    ...
                    foreach (var par in typeof(Modbus33LogDTO).GetProperties())
                    {
                        ...
                        if (display.Visible)
                        {
                            try
                            {
                                str += string.Format(",{0}", par.GetValue(Modbus33Log_).ToString());
                            }
                            catch
                            {
                                str += string.Format(",{0}", "-");
