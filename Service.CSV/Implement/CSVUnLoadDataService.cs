...
{
    internal class CSVUnLoadDataService : ICSVUnLoadDataService
    {
        readonly string FolderPath = CSVServiceFactory.BasicFolderPath + @"UnLoadDataLog\";

        static readonly object Token = new object();

        public ActResult Log(UnLoadDataLogDTO UnLoadDataLog_)
        {
            lock (Token)
            {
                try
                {
                    ...
                    if (!File.Exists(fileName))
                    {
                        ...
                        foreach (var par in typeof(UnLoadDataLogDTO).GetProperties())
                        {
                    ...
                    foreach (var par in typeof(UnLoadDataLogDTO).GetProperties())
                    {
                        ...
                        {           
                            try                         
                            {
                                str += string.Format(",{0}", par.GetValue(UnLoadDataLog_).ToString());
                            }
                            catch
                            {
                                str += string.Format(",{0}", "-");
