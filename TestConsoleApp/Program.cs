...
namespace TestConsoleApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var newCheckItem = new CheckItemObject(); foreach (var property in typeof(CheckItemObject).GetProperties()) property.SetValue(newCheckItem, true);
            var r6 = SettingController.SetCheckItemValue(IniPath, newCheckItem);            
            r1 = SettingController.GetCheckItemValue(IniPath);
            if (!r1.Result)
                Console.Write("警告", "取得檢查項目失敗將使用預設值。 請前往設定頁面確認當前檢查值為何。");
            ...
            therm.TC03 = 100;
            var normalisfail = service.ParamterComparison(lotcode, opid, checkItem, therm, mos31, mos32, mos33, ws);

            if (!normalisfail.Result)
            {
                var AlarmMsgs = normalisfail.Value;
            }

            therm.TC03 = 99999;
            var maxisfail = service.ParamterComparison(lotcode, opid, checkItem, therm, mos31, mos32, mos33, ws);

            if (!maxisfail.Result)
            {
                var AlarmMsgs = maxisfail.Value;
            }


            therm.TC03 = -1;
            var minisfail = service.ParamterComparison(lotcode, opid, checkItem, therm, mos31, mos32, mos33, ws);

            if (!minisfail.Result)
            {
                var AlarmMsgs = minisfail.Value;
            }
